...
<body>
    <div class="container mt-5">
        <h1 class="text-center mb-5">Edit Daftar Ikan</h1>

        <!-- Tombol Tambah Menu -->
        <div class="text-end mb-3">
            <a href="{{ route('tambah_ikan') }}" class="btn btn-success">Tambah Menu</a>
        </div>
        ...
        <table class="table table-bordered">
            ...
            @foreach($ikans as $key => $ikan)
                <tr>
                    ...
                    <td>{{ $ikan->harga }}</td>
                    <td>
                        <form action="{{ route('hapus_ikan', $ikan->id) }}" method="POST" style="display:inline;">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn-danger btn-sm" onclick="return confirm('Apakah Anda yakin ingin menghapus ikan ini?')"><i class="bi bi-trash">Hapus</i></button>
                        </form>
